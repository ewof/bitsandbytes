...
hip: $(BUILD_DIR)
	$(HIPCC) -std=c++14 -c -fPIC --amdgpu-target=${HCC_AMDGPU_TARGET} $(HIP_INCLUDE) -o $(BUILD_DIR)/ops.o -D NO_CUBLASLT $(CSRC)/ops.cu
	$(HIPCC) -std=c++14 -c -fPIC --amdgpu-target=${HCC_AMDGPU_TARGET} $(HIP_INCLUDE) -o $(BUILD_DIR)/kernels.o -D NO_CUBLASLT $(CSRC)/kernels.cu
